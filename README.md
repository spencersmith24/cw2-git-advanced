...
Make 3 different commits to the file _scratch.txt_ so that its contents are updated and the `git log` is now three longer. Push those commits with `git push`. Assume we want to delete these changes—use `git reset HEAD~3` to delete the last three commits. Ensure that the `git log` now contains none of those commits. You'll get a warning that there are unstaged changes after the reset—that's because it only resets the commit history, and not your working tree. Thus, you can use `git restore scratch.txt` to reset that file back to the state of the latest commit. (You can do these both in one step by appending `--hard` to the end of your `git reset` command, which will cause it to also update your working tree.) Then, use `git push origin --force` to update the commit history on remote. The last three commits have been reset without an extra commit needed to reset them, and without the possibility of a revert conflict!

### Getting rid of local changes
Make some changes without committing them. Say we don't want to keep these changes—either they don't work, or someone has just pushed to remote and we just want to take that change. Use `git restore scratch.txt` (just like the last step) or `git reset --hard` to delete those changes—not providing a commit to reset to (in our last example, HEAD~2, or two commits behind the HEAD commit [three commits ago]) just means reset to the latest commit, which won't do anything unless you provide `--hard`, which tells Git to also update the working tree.<br>
> **What about if I've already committed**?<br>
Refer to the previous step on resetting commit history to use `git reset <number-of-commits> --hard` to remove local commits and update the working tree, and then you can pull to update your working tree back to normal.
...
> **Cleanup**<br>
> Commit the changes applied from the stash. Use `git switch master` and `git merge second` to merge that branch back into the master branch, then `git branch -d second` to remove it.
